...
func main() {
	...
	initGRPCPtr := initCommand.String("grpc", "dns:///0.0.0.0:9090", "The grpc endpoint of the sekaid. (Required)")
	...
	if len(os.Args) >= 2 {
		...
		case "init":
			...
			if initCommand.Parsed() {
				...
				grpcEndpoint := *initGRPCPtr
				fmt.Println("grpcEndpoint", grpcEndpoint)
				return
			}
		case "start":
