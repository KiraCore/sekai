...
// GetCmdQueryAddressesByBlacklistedRole - list all kira addresses by a specific blacklisted role (address does NOT have to be a Councilor)
func GetCmdQueryAddressesByBlacklistedRole() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "blacklisted-role-addresses [role]",
		Args:  cobra.ExactArgs(1),
		Short: "Query all kira addresses by a specific blacklisted role (address does NOT have to be a Councilor)",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Query all kira addresses by a specific blacklisted role (address does NOT have to be a Councilor).

Example:
$ %[1]s query gov blacklisted-role-addresses [role]
`,
				version.AppName,
			),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := queryClient.QueryAddressesByBlacklistedRole(
				context.Background(),
				&types.QueryAddressesByBlacklistedRole{
					Role: uint32(role),
				},
			)
