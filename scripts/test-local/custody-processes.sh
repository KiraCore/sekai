...
sendFailTokens tester1 tester4 7 ukex 150 ukex
TESTER1_BALANCE_END=$(showBalance tester1 ukex)
POOL=$(getCustodyPool "$ACCOUNT1_ADDRESS")

echoInfo "INFO: Custody pool: $POOL"

[ "$POOL" == "null" ] && echoErr "ERROR: Expected not empty pool" && exit 1
[ "$TESTER1_BALANCE_START" != "$TESTER1_BALANCE_END" ] && echoErr "ERROR: Expected tester1 account balance to be '$TESTER1_BALANCE_START', but got '$TESTER1_BALANCE_END'" && exit 1
